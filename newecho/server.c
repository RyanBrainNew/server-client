...
int already_running(const char *szLockFilePath)
{
	int fd_ar;
	char szBuf[256] = { 0x00 };
	fd_ar = open(szLockFilePath, O_RDWR|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
	if (fd_ar < 0){
	...
	write(fd_ar, szBuf, strlen(szBuf)+1);
	return 0;
}
...
int getNum(char* s)
{
	if(strncmp(s,"1",1)==0) 		return 1;  	 
	else if(strncmp(s,"2",1)==0) 	return 2;  	
	else if(strncmp(s,"3",1)==0)	return 3;   
	else 				 			return -1;
			...
			for(fd = 0; fd < FD_SETSIZE; fd++)
			{
				...
				if(FD_ISSET(fd,&testfds))
				{
					...
					else
					{
						...
						//FIONREAD就是返回缓存区有多少字节能到缓冲区里面，有多少字節
						//要被读取。值放在 nread裡面了。
